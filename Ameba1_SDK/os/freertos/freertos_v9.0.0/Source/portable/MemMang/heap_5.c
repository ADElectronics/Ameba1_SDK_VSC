...
void *pvPortMalloc(size_t xWantedSize) {
	...
	(void) xTaskResumeAll();
	if (pvReturn == NULL) {
		DBG_8195A("ram_alloc(%d): freeSpace(%d)!\n", xWantedSize,
				xFreeBytesRemaining);
	} else {
//		DBG_8195A("ram_alloc:%p[%d]\n", pvReturn , xWantedSize);
	}
...
static void vPortDefineHeapRegions(const HeapRegion_t * const pxHeapRegions) {
	...
	{
#ifdef CONFIG_PLATFORM_8195A
		if (pxHeapRegion->pucStartAddress > (uint8_t *) 0x20000000 && chip_id >= CHIP_ID_8711AN && chip_id <= CHIP_ID_8711AF)
		{
