...
//	_memset(SpicInitParaAllClk, 0, sizeof(SpicInitParaAllClk));
	*(uint32_t *)(&SpicInitParaAllClk[0][0].BaudRate) = 0x01310202; // patch
	*(uint32_t *)(&SpicInitParaAllClk[1][0].BaudRate) = 0x11311301; // patch
//	*(uint32_t *)(&SpicInitParaAllClk[2][0].BaudRate) = 0x21311301; // patch
#ifdef CONFIG_SDR_EN
	SPI_FLASH_PIN_FCTRL(ON);
/*
//	uint8_t SpicBaudRate = CPU_CLK_TYPE_NO - 1 - ((HAL_SYS_CTRL_READ32(REG_SYS_CLK_CTRL1) >> 4) & 7);
	uint8_t SpicBaudRate = 3; // HAL_SYS_CTRL_READ32(REG_SYS_CLK_CTRL1) >> 4) & 7;
...
//	uint8_t ChipId = HalGetChipId();
	if (ChipId >= CHIP_ID_8195AM) {
		...
		if((uint32_t)__sdram_bss_end__-(uint32_t)__sdram_bss_start__ > 0)
			memset(__sdram_bss_start__, 0, (uint32_t)__sdram_bss_end__-(uint32_t)__sdram_bss_start__);
	}
