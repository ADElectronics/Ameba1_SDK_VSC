...
// Warning! If Start CLK > 100 MHz -> Errors SPIC function in Ameba SDK!
#define DEFAULT_BOOT_CLK_CPU 1
...
static uint32_t InitTabParaAllClk[3 * CPU_CLK_TYPE_NO] = {
		// SIO
		0x01310102,	// 72t/byte
...
static int BOOT_RAM_TEXT_SECTION SetSpicBitMode(uint8_t BitMode)
{
	PSPIC_INIT_PARA pspic = &SpicInitParaAllClk[BitMode][((HAL_SYS_CTRL_READ32(REG_SYS_CLK_CTRL1) >> 4) & 7)];
	if(pspic->Mode.Valid)
	{
		// Disable SPI_FLASH User Mode
		HAL_SPI_WRITE32(REG_SPIC_SSIENR, OFF);
    ...
    if(!SpicCmpDataForCalibrationRtl8195A())
	{
    	...
    	for(uint8_t BaudRate = 1; BaudRate < 4; BaudRate++)
		{
    		for(uint8_t RdDummyCyle = 0; RdDummyCyle < 63; RdDummyCyle++)
			{
    			// Disable SPI_FLASH User Mode
    		    HAL_SPI_WRITE32(REG_SPIC_SSIENR, OFF);
...
static int BOOT_RAM_TEXT_SECTION InitSpic(uint8_t SpicBitMode)
{
	_memset(SpicInitParaAllClk, 0, sizeof(SpicInitParaAllClk));
	uint32_t * ptr = InitTabParaAllClk;
	uint8_t x;
	for(x = 0; x < SpicMaxMode; x++)
	{
		*(uint32_t *)&SpicInitParaAllClk[SpicOneBitMode][x].BaudRate = ptr[0];
		*(uint32_t *)&SpicInitParaAllClk[SpicDualBitMode][x].BaudRate = ptr[CPU_CLK_TYPE_NO];
		*(uint32_t *)&SpicInitParaAllClk[SpicQuadBitMode][x].BaudRate = ptr[CPU_CLK_TYPE_NO]; //  MXIC not QIO
		ptr++;
	}
	ACTCK_FLASH_CCTRL(1);
	SLPCK_FLASH_CCTRL(1);
...
static uint8_t INFRA_START_SECTION _Get_ChipId()
{
	uint8_t ChipId = CHIP_ID_8710AF;
	if (HALEFUSEOneByteReadROM(HAL_SYS_CTRL_READ32(REG_SYS_EFUSE_CTRL), 0xF8, &ChipId, L25EOUTVOLTAGE) != 1)
...
/*-------------------------------------------------------------------------------------
 ...
 --------------------------------------------------------------------------------------*/
static unsigned int BOOT_RAM_TEXT_SECTION flashcpy(unsigned int faddr, void *dist, unsigned int size)
{
	union
	{
		unsigned char uc[4];
...
static const uint32_t tab_seg_def[] = {
...
static uint32_t BOOT_RAM_TEXT_SECTION get_seg_id(uint32_t addr, int32_t size)
{
	uint32_t ret = SEG_ID_ERR;
...
static uint32_t BOOT_RAM_TEXT_SECTION load_img2_head(uint32_t faddr, PIMG2HEAD hdr)
{
	flashcpy(faddr, hdr, sizeof(IMG2HEAD));
	uint32_t ret = get_seg_id(hdr->seg.ldaddr, hdr->seg.size);
...
static uint32_t BOOT_RAM_TEXT_SECTION load_segs(uint32_t faddr, PIMG2HEAD hdr, uint8_t flgload) {
	uint32_t fnextaddr = faddr;
	uint8_t segnum = 0;
	while (1)
	{
		uint32_t seg_id = get_seg_id(hdr->seg.ldaddr, hdr->seg.size);
		if (flgload && (seg_id == SEG_ID_SRAM || seg_id == SEG_ID_TCM || seg_id == SEG_ID_SDRAM))
		{
...
static int BOOT_RAM_TEXT_SECTION loadUserImges(int imgnum)
{
	IMG2HEAD hdr;
	int imagenum = 1;
	uint32_t faddr = 0xb000; // start image2 in flash
	...
	while (1)
	{
		...
		{ // есть подпись  "RTKW" + RUN или SWP, сегмент != unknown
...
static uint8_t BOOT_RAM_TEXT_SECTION IsForceLoadDefaultImg2(void)
{
	uint8_t gpio_pin[4];
...
#ifdef CONFIG_SDR_EN
	uint8_t ChipId = _Get_ChipId();
	if (ChipId < CHIP_ID_8195AM)
	{
#endif
		SDR_PIN_FCTRL(OFF); // SDRAM Off
		LDO25M_CTRL(OFF);
#ifdef CONFIG_SDR_EN
		// Flag SDRAM Init or None
...
#ifdef CONFIG_SDR_EN
	if ((HAL_PERI_ON_READ32(REG_SOC_FUNC_EN) & BIT(21)) == 0)
	{ // Flag SDRAM No ReInit?
		...
		{
			uint32_t *ptr = (uint32_t *)SDR_SDRAM_BASE;
			uint32_t tt = 0x55AA55AA;
...
//				ptr[i] = 0;
		...
		if(flg && *((uint32_t *)0x1FFF0000) == 0x12345678)
		{
			*((volatile uint32_t *)0x1FFF0000) = 0x87654321;
			uint32_t tt = 0x03ffffff; // ~7 sec
			DBG_8195A("Waiting for SDRAM to load...\n");
//			__asm__ __volatile__ ("cpsid f\n");
			while(*((volatile uint32_t *)0x1FFF0000) == 0x87654321 && tt--);
//			__asm__ __volatile__ ("cpsie f\n");
			if(*((volatile uint32_t *)0x1FFF0000) == 1)
