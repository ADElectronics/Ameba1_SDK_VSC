 ...
 ******************************************************************************/
static err_t tcpsrv_server_sent(void *arg, struct tcp_pcb *pcb, u16_t len) {
	int8_t ret_err = ERR_OK;
 ...
 * Returns      : err_t
 ******************************************************************************/
static err_t recv_trim_bufi(TCP_SERV_CONN * ts_conn, uint32_t newadd)
{
	uint32_t len = 0;
//	while(ts_conn->flag.busy_bufi) vTaskDelay(1);
	...
	if(newadd) {
		...
		if(ts_conn->pbufi != NULL) { // уже есть какой-то буфер
			if ((ts_conn->flag.rx_buf) && ts_conn->cntri < ts_conn->sizei) { // буфер не пуст
				...
				if(ts_conn->cntri >= newadd) { // кол-во обработанных байт (пустого места в буфере) больше чем дополнение
					memcpy(ts_conn->pbufi, &ts_conn->pbufi[ts_conn->cntri], len ); // скопировать необработанную часть в начало буфера
					ts_conn->sizei = len;
		...
		if((!ts_conn->flag.rx_buf) || ts_conn->cntri >= ts_conn->sizei)  { // буфер обработан
			ts_conn->sizei = 0;
			if (ts_conn->pbufi != NULL) {
				free(ts_conn->pbufi);  // освободить буфер.
				ts_conn->pbufi = NULL;
			};
		}
			...
			memcpy(ts_conn->pbufi, &ts_conn->pbufi[ts_conn->cntri], len );
			ts_conn->sizei = len;
			ts_conn->pbufi = (uint8_t *)os_realloc(ts_conn->pbufi, len + 1);	//mem_trim(ts_conn->pbufi, len);
			if(ts_conn->pbufi == NULL) {
				ts_conn->flag.busy_bufi = 0; // обработка bufi окончена
 ...
 ******************************************************************************/
static err_t tcpsrv_server_recv(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err) {
...
//	if(syscfg.cfg.b.hi_speed_enable) set_cpu_clk();
	...
	if(p->tot_len) {
		...
		uint32_t len = pbuf_copy_partial(p, &ts_conn->pbufi[ts_conn->sizei], p->tot_len, 0);
		ts_conn->sizei += len;
 ...
 *******************************************************************************/
static err_t tcpsrv_server_accept(void *arg, struct tcp_pcb *pcb, err_t err) {
...
//	*(uint16_t *) &ts_conn->flag = 0; //zalloc
 ...
 *******************************************************************************/
TCP_SERV_CFG * tcpsrv_server_port2pcfg(uint16_t portn) {
	TCP_SERV_CFG * p;
 ...
 *******************************************************************************/
int tcpsrv_close_all_tcp_pcb(void)
{
	struct tcp_pcb *pcb;
	int ret = 0;
	for (pcb = tcp_active_pcbs; pcb != NULL; pcb = pcb->next) {
 ...
 *******************************************************************************/
void tcpsrv_delete_all_tm_tcp_pcb(void)
{
	struct tcp_pcb *pcb;
	for (pcb = tcp_tw_pcbs; pcb != NULL; pcb = pcb->next) {
