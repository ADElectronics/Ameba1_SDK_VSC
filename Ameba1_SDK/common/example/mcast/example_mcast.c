...
static void example_mcast_thread(void *param)
{
	...
	if(err == 0) {
		...
		err = setsockopt(sock, IPPROTO_IP, IP_ADD_MEMBERSHIP, &imr, sizeof(imr));
	...
	if(err == 0) {
		...
		err = setsockopt(sock, IPPROTO_IP, IP_MULTICAST_IF, &intfAddr, sizeof(struct in_addr));
	...
	if(err == 0) {
		...
		while(1) {
			...
			if((packetLen = recvfrom(sock, &packet, sizeof(packet), 0, &from, &fromLen)) >= 0) {
			...
			if(packetLen > 0) {
				...
				if((sendLen = sendto(sock, packet, packetLen, 0, &to, sizeof(struct sockaddr))) < 0)
